...
def answer_generation(query, graph_result): 
    ...
    if graph_result:
        ...
        final_prompt = f"""You are a helpful question-answering agent. Your task is to analyze 
        and synthesize information. Given the user's query: {query}, provide a meaningful and efficient answer.
        """
...
try:
    ...
    query = sys.argv[1] if len(sys.argv) > 1 else "How do I fix PyDev 'Undefined variable from import' errors?"
    graph_result = graph_cycher_QA_chain(query)

    html_response = answer_generation(query, graph_result)
    print(html_response) 
